...
quant_group.add_argument('--qe-bits-wts', '--qebw', type=int, default=4, metavar='NUM_BITS',
...
def main():
    ...
    args.dataset = 'cifar10' if 'cifar' in args.arch else 'mnist'
    args.num_classes = 1000 if args.dataset == 'imagenet' else 10
    ...
    if args.resume:
        model, compression_scheduler, start_epoch = apputils.load_checkpoint(
            model, chkpt_file=args.resume,optimizer=optimizer)
...
def test_print(test_loader, model, criterion, loggers, activations_collectors, args):
    """Model Test Print"""
    msglogger.info('--- test ----print-------------')
    with collectors_context(activations_collectors["test"]) as collectors:
        distiller.log_activation_statsitics(-1, "test", loggers, collector=collectors['sparsity'])
    return _validate_print(test_loader, model, criterion, loggers, args)
...
def _validate_print(data_loader, model, criterion, loggers, args, epoch=-1):
    """Execute the validation/test loop."""
    losses = {'objective_loss': tnt.AverageValueMeter()}
    classerr = tnt.ClassErrorMeter(accuracy=True, topk=(1, 5))
    ...
    for validation_step, (inputs, target) in enumerate(data_loader):
        ...
        #with PytorchNoGrad():
            ...
            file_act = inputs.data.cpu().numpy().reshape(-1,1)
            file_res = target.data.cpu().numpy().reshape(-1,1)
            #np.savetxt('test_act'+str(validation_step)+'.csv',file_act,delimiter=',',fmt="%f")
            #np.savetxt('test_res'+str(validation_step)+'.csv',file_res,delimiter=',',fmt="%f")
...
def evaluate_model(model, criterion, test_loader, loggers, activations_collectors, args):
    ...
    for k,v in params.items():
        file = params[k].cpu().numpy().reshape(-1,1)
        np.savetxt(k+'.csv',file,delimiter=',',fmt="%.5f")    
    ...
    #top1, _, _ = test(test_loader, model, criterion, loggers, activations_collectors,args)
    
    ##print activation
    test_print(test_loader, model, criterion, loggers, activations_collectors,args)
